...
public class HRN implements Runnable{
    ...
    public void show() throws InterruptedException {
        ...
        output.append("当前执行高响应比优先算法").append("\n");
    ...
    public void sort(){
        ...
        while (!readyQueue.isEmpty()){
            PCB temp = readyQueue.poll();
            double waitTime = CPUTime-temp.getComingTime();
            temp.setResponseRatio((waitTime+temp.getNeedTime())/temp.getNeedTime());
            System.out.println(temp.getResponseRatio());
            list.add(temp);
        }
        for(int i=0;i<list.size();i++){
            ...
            for (int j=0;j<list.size();j++){
                if(list.get(j).getResponseRatio() > max){
                    max = list.get(j).getResponseRatio();
                    select = j;
                }
            }
            readyQueue.add(list.get(select));
            list.get(select).setResponseRatio(0);
        }
        //System.out.println(readyQueue);
    }
